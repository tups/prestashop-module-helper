...
class UploadHelper
{
    ...
    function upload($file, $lang = 'en_GB')
    {
        ...
        if ($this->gdversion()) {
            if (imagetypes() & IMG_GIF) {
                ...
                $this->image_supported['image/svg+xml'] = 'svg';
            }
        ...
        if (!is_array($file)) {
            if (empty($file)) {
                ...
                if (substr($file, 0, 4) == 'php:' || substr($file, 0, 5) == 'data:' || substr($file, 0, 7) == 'base64:') {
                    ...
                    $hash = $this->temp_dir() . md5($file . rand(1, 10000000));
            ...
            if (!$this->file_src_mime || !is_string($this->file_src_mime) || empty($this->file_src_mime) || strpos($this->file_src_mime, '/') === FALSE) {
                if ($this->mime_magic) {
                    ...
                    if (function_exists('mime_content_type')) {
                        ...
                        if($mime_from_browser == 'image/svg+xml'){
                            $this->file_src_mime = 'image/svg+xml';
                        }
    ...
    function temp_dir()
    {
        ...
        if (!$dir) return '';
        $slash = (mb_strtolower(substr(PHP_OS, 0, 3)) === 'win' ? '\\' : '/');
    ...
    function process($server_path = null)
    {
        ...
        if ($this->processed) {
            ...
            if ($this->file_safe_name) { // formats the name
                if(strlen($this->file_dst_name_body) > 50 && substr($this->file_dst_name_body,-5) != '_mini'){
                    $this->file_dst_name_body = substr($this->file_dst_name_body,0,50);
                }

                $this->file_dst_name_body = (strtr((mb_strtolower($this->file_dst_name_body)), array(
                    "À" => "a",
                    "Á" => "a",
                    "Â" => "a",
                    "à" => "a",
                ...
                $this->file_dst_name_body = preg_replace(array('/\s/', '/\.[\.]+/', '/[^\w_\.\-]/'), array('_', '_', '_'), $this->file_dst_name_body);
            ...
            if ($image_manipulation) {
                ...
                if ($this->processed && $image_src) {
                    ...
                    if (!empty($this->image_text)) {
                        ...
                        if (!is_numeric($this->image_text_font) && strlen($this->image_text_font) > 4 && substr(mb_strtolower($this->image_text_font), -4) == '.gdf') {
